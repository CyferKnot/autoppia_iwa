...
### CONFIRMATION GUIDELINES FOR TEST HARNESS (UNCONFIRMED ACTIONS)

To streamline validation and reduce false negatives, the following rules may be used to **auto-confirm** assertions:

✅ **Auto-Confirm Criteria:**
- Selector includes one of:
  - `#movie-title`, `#movie-list`, `#success-message`, `.movie-title`, `.movie-rating`, `.movie-genre`, `.movie-director`
- `text_to_assert` is short (≤ 3 words), and contains:
  - A known movie title (e.g., "Inception", "The Matrix")
  - A common dummy value like "Test Movie"
- `text_to_assert` is a numeric rating or year (e.g., `9.3`, `2023`, `120`)

⚠️ **Manual Inspection Needed If:**
- Selector targets legal/disclaimer content (e.g., `#copyright-notice`)
- `text_to_assert` contains full sentences, URLs, or comma-separated values
- Value includes punctuation beyond periods or numbers
...
{{
  ...
  "reasoning": "Validates that the correct subject value was submitted in the contact form."
}}
...
You must return a raw JSON array of `CheckEventTest` objects. DO NOT wrap it in code fences, markdown, or any other text.
