...
class ConcurrentEvaluator(IEvaluator):
    ...
    async def _evaluate_single_task_solution(self, task: Task, task_solution: TaskSolution) -> EvaluationResult:
        ...
        if not actions:
            ...
            logger.info(f"[EVAL DEBUG] Task {task.id} | Agent {web_agent_id} | No actions provided")
            log_jsonl({
        ...
        try:
            ...
            # Text log (stdout)
            logger.info(f"[EVAL DEBUG] Task {task.id} | Agent {web_agent_id} | Final Score: {final_score:.2f}")
            logger.info(f"[EVAL DEBUG] Tests Passed: {tests_passed_count}/{num_tests}")
            logger.info(f"[EVAL DEBUG] Raw Score: {raw_score:.2f}, Random Clicker Score: {random_clicker_score:.2f}")
            logger.info(f"[EVAL DEBUG] Actions Executed: {len(actions)}")
            for i, action in enumerate(actions, 1):
                logger.info(f"[ACTION EXECUTION] #{i} → {action}")
            ...
            log_jsonl({
                ...
                "score": {
                    "raw": raw_score,
                    "final": final_score,
                    "random_clicker": random_clicker_score
                },
            ...
            logger.error(f"[EVAL ERROR] Task {task.id} | Agent {web_agent_id} | Exception: {str(e)}")
            log_jsonl({
                "event": "evaluation_result",
                "task_id": str(task.id),
                "web_agent_id": web_agent_id,
